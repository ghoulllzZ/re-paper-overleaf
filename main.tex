...
\title[Beyond Majority Vote]{Beyond Majority Vote: Good-Outlier Recognition and Roundtable Consensus for Compact, Higher-Precision Per-Requirement Issue Lists}
...
Finally, from the team scores, evidence spans, and rewrite suggestions after consensus, we apply a selection threshold $\tau_{sel}$ (a team-score cutoff, distinct from the outlier sensitivity threshold $\theta$ of Stage~2) to extract the final high-quality issue set:
\begin{equation}
\mathit{final\_problems} = \left\{ (i,d) \;\middle|\; S_{issue}(i,d) \ge \tau_{sel} \right\}
\label{eq:final}
\end{equation}
...
To evaluate final issue-list quality we build ground truth (GT) by expert cross-blind annotation. Two experts with several years of RE experience independently annotate each $(item,type)$ pair for defect presence; disagreements are arbitrated by a third senior expert. Initial two-expert agreement before adjudication is low (Cohen's $\kappa=0.14$; Krippendorff's $\alpha=0.13$), reflecting the inherent subjectivity of fine-grained per-dimension requirements-quality judgments; we return to this limitation in Section~\ref{sec:validity}. Per RQ: RQ1 uses ordinal Krippendorff's $\alpha$ for initial-score agreement and the frequency/distribution of outlier events; RQ2 computes the normalized argument-quality score $Q$ and the share of events triggering ``hard fail'' (missing evidence, dimension mismatch, or non-executable rewrite); RQ3, given the severe class imbalance across the four defect types, adopts Macro-F1 as the core metric, and also reports Macro-Precision, Macro-Recall, and the issue-set Jaccard similarity between adjacent roundtable rounds.
...
Statistical analysis is blocked by document to avoid Simpson's paradox from directly mixing heterogeneous items. We use $\alpha=0.05$ as the significance level and report non-parametric effect sizes alongside significance. For RQ1 (disagreement structure) we use a Kruskal--Wallis test on whether within-document outlier rates differ across U/A/C/V; for RQ2 (discrimination) a Mann--Whitney U test compares good vs.\ bad outliers on $Q$ and sub-dimensions, with Cliff's $\delta$ effect sizes; for RQ3 (performance and ablation), since each document is evaluated under all four treatments, a global Friedman test compares Macro-F1 and, if significant, Holm--Bonferroni-corrected pairwise comparisons follow, together with Jaccard curves and score-fluctuation decay plots to visualize convergence.
...
Conclusion validity concerns whether the statistical analysis supports the inferences. The main threat is statistical bias from document heterogeneity (large differences in size and defect density); mixing all items to compute $p$-values risks Simpson's paradox. \textit{Mitigation:} we strictly use ``document'' as the paired-comparison block, apply robust non-parametric tests (Friedman and paired Wilcoxon signed-rank), and mandatorily report non-parametric effect sizes (Cliff's $\delta$). In addition, the Correctness (C) dimension has only $4$ positive ground-truth units out of $978$ ($0.41\%$), so its per-dimension F1 is near-degenerate; any C-specific observation---including the $0\%$ C retention in the final issue lists---should be read as a data-sparsity artefact rather than a substantive finding, and macro-averaged scores are correspondingly sensitive to this near-empty class. Finally, the low initial inter-annotator agreement (Section~\ref{sec:metrics}) means the ground truth itself carries measurement uncertainty, so absolute metric values should be interpreted with caution and comparative (paired, within-document) evidence should be given more weight.
